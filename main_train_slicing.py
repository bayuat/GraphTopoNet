...
from BedTopoDataset_slicing import BedTopoDataset
...
def main():
    ...
    dataset = BedTopoDataset(inputs, target_bed, radar_mask_tensor, 
                             patch_size=16, stride=8, slice_axis='horizontal',
                             num_slices=30)
    ...
    for epoch in range(num_epochs):
        ...
        model.train()
        train_loss = 0.0
        for input_patch, target_patch, radar_mask_patch, _ in train_loader:        
        ...
        val_loss = 0.0
        with torch.no_grad():
            for input_patch, target_patch, radar_mask_patch, _ in val_loader:
        ...
        if val_loss < best_loss:
            ...
            best_loss = val_loss
            torch.save(model.state_dict(), './saved_models/best_bayesian_uncertainty_gcn_model_top_right_sliced.pth')
            counter = 0
        else:
            counter += 1
